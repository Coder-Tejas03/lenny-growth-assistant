...
│   Grounded insights from 272 episodes of Lenny's Podcast         │
│                                                                  │
│  ┌─────────────────────────┐    ┌─────────────────────────────┐  │
│  │ 🎯 Product-Market Fit   │    │ 🔄 Growth Loops vs Funnels   │  │
│  │ Signals, 40% rule       │    │ Compounding viral engines   │  │
│  └─────────────────────────┘    └─────────────────────────────┘  │
│  ┌─────────────────────────┐    ┌─────────────────────────────┐  │
│  │ 📈 B2B Pricing Strategy │    │ 🚀 0 to 1 User Acquisition  │  │
│  │ Value metrics & tiers   │    │ Cold start playbooks        │  │
│  └─────────────────────────┘    └─────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────┘
```

The empty state provides a concise explanation and 4 curated starter prompt cards covering high-frequency product management and growth disciplines.

### Session List & Title Generation
- **Instant Optimistic Titles:** When a user sends their first message, `lib/title-generator.ts` generates a clean, readable title (e.g. *"Product-Market Fit Signals"*) instantly without blocking on background LLM title generation.
- **Session Management:** Users can start a new chat (`+ New Chat`), rename an existing session inline, or delete a session with a confirmation modal.
- **Active State:** The active session is highlighted with a subtle accent border and background tint.
