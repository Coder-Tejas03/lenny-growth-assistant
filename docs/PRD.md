...
- OpenAI `text-embedding-3-small` is the default embedding model (1,536 dimensions). The entire archive of 272 episodes and 7,587 chunks is embedded and indexed in PostgreSQL `pgvector` with HNSW (`m=16, ef_construction=64`).
- An instant pre-computed database seed dump (`scripts/restore_db.sh`) is supplied with the repository, allowing evaluators to restore the complete archive in ~5 seconds with zero OpenAI credit spend.
- The local machine is CPU-only and cannot run a large model comfortably. The mandatory Ollama demonstration uses `qwen2.5:1.5b` (benchmarked at ~17 tokens/s on CPU with ~0.3s warm TTFT).
- A user with more capable hardware may select a 7B/8B Ollama model through configuration without code changes.

**Scope choices and trade-offs:** The system uses PostgreSQL 16 + pgvector, a single FastAPI application boundary, and an internal Pi Coding Agent runtime rather than a separate vector store, Anthropic gateway, or heavy microservice cluster. This keeps the system reproducible and affordable while preserving explicit provider and skill boundaries. Multi-turn conversational continuity is powered by a fast `QueryRewriter` on Turn 2+, resolving ambiguous pronouns into standalone semantic queries before vector search.
...
## FR-04 — Multi-turn Conversations & Conversational Memory
 
The assistant shall support multi-turn conversational follow-ups and preserve context across turns within a session.

Example:

> **Turn 1 User:** How do I know if my startup has genuine product-market fit? What signals should I look for?  
> **Turn 1 Assistant:** *(Retrieves Sean Ellis 40% disappointment metric and retention cohorts)*  
> **Turn 2 User:** And how do I collect this customer feedback data?

**Implementation Specification:**
1. On Turn 2+, the system invokes a lightweight, low-latency `QueryRewriter` (`gpt-4o-mini`).
2. It analyzes the recent chat history and rewrites ambiguous pronouns ("this", "that", "it") into a self-contained semantic search query (e.g. *"How to collect customer feedback data for the 40% product-market fit disappointment survey"*).
3. The rewritten query is sent to `pgvector`, guaranteeing high semantic recall without carrying noisy dialogue tokens into the vector search.
4. If a query is already self-contained, the rewriter preserves it unchanged.

Conversations shall persist in PostgreSQL so that users can resume or inspect past sessions at any time.
